...
def build_grid(
    ...
    if stop_distance is not None and stop_type is None:
        # set stop_type to 'limit' by defaults
        stop_type = "limit"
...
def HIGH(df, n):
   ...
   high = df.High.rolling(n).max()[-1]
   return high

def LOW(df, n):
   
   low=df.Low.rolling(n).min()[-1]
   return low
...
def MACD_SIGNAL(df, n_fast = 12, n_slow = 26, n_signal = 9):

    exp1 = df.Close.ewm(span=n_fast).mean()
    exp2 = df.Close.ewm(span=n_slow).mean()
    macd = exp1 - exp2
...
def FDI(data):
    if len(data) >= N:
        ...
        for Period in range(1, N):
            if (HH - LL) > 0:
                diff = (data[-N + Period] - LL) / (HH - LL)
                if Period > 1:
                    length += np.sqrt(np.square(diff - pdiff) + (1 / np.square(N)))
                pdiff = diff
        
        if length > 0:
            FDI = 1 + (np.log(length) + np.log(2)) / np.log(2 * N)
        else:
            FDI = 0
...
def BBANDS(df, n = 14, std = 2.0):

    bbands = ta.bbands(df.Close, length=n, std=std)
    return bbands
       
def isStar(l, high, highdiff, open, close, low, bodydiff, lowdiff, ratio1, ratio2):
...
def Stochastic_RSI(data: pd.DataFrame, window_length=14) -> pd.Series:
    ...
    df_ = data.copy()
    rsi = RSI(df_)
    return (rsi - rsi.rolling(window_length).min()) / (rsi.rolling(window_length).max() - rsi.rolling(window_length).min()) * 100
...
def ZLSMA(df, n):
   ...
   zlma = ta.zlma(df.Close, length=n)
   return zlma

###################### Ichimoku System ########################

def ichimoku_cloud(df, conversion_periods=9, base_periods=26, lagging_span_periods=52, displacement=26):
    # Donchian Channel calculation function
    def donchian(high, low, len):
        return ((high.rolling(len).max()) + (low.rolling(len).min())) / 2

    # Calculate Conversion Line
    df['conversion_line'] = donchian(df['High'], df['Low'], conversion_periods)

    # Calculate Base Line
    df['base_line'] = donchian(df['High'], df['Low'], base_periods)

    # Calculate Leading Span A
    df['lead_line_1'] = ((df['conversion_line'] + df['base_line']) / 2).shift(displacement)

    # Calculate Leading Span B
    df['lead_line_2'] = donchian(df['High'], df['Low'], lagging_span_periods).shift(displacement)

    # Identify Cloud Bounds
    df['above_cloud'] = df[['lead_line_1', 'lead_line_2']].max(axis=1)
    df['below_cloud'] = df[['lead_line_1', 'lead_line_2']].min(axis=1)

    return df

## ADX (Flat Market Indicator)
