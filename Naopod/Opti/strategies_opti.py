...
def DivergenceIchimoku(df,
    ...
    if not small_sl_tp: 
        ...
        if timeframe == 'M5':
            ...
            df['tp_long'] = df.Close + 1.1*(df.Close - (df.Low.rolling(2).min() - df.atr/4)) 
            df['tp_short'] = df.Close - 1.1*((df.High.rolling(2).max() + df.atr/4) - df.Close)
        elif timeframe == 'M15':
            df['sl_long'] = df.Low.rolling(2).min() - df.atr/4
            df['sl_short'] = df.High.rolling(2).max() + df.atr/4
