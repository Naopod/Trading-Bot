from data_graph import MergedDF

'''Filtering of signals'''

def FinalStrategy(symbol):
    ...
    for i in range(len(df)):
        ...
        if symbol in ['TEST', 'TEST']:
            ...
            row = df.iloc[i]
            signals = [int(row['signal_M5_3']), int(row['signal_M5_2'])]
            sls = [row['sl_M5_3'], row['sl_M5_2']]
            tps = [row['tp_M5_3'], row['tp_M5_2']]
